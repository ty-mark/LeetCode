    ...
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            ...
            /* All status:
                ...
                So we only need to care about alive ones after ghe game.
            */
            
            if (board[i][j] == 1 && lives >= 2 && lives <= 3) { // previously live and keep alive
                board[i][j] = 3; // Make the 2nd bit 1: 01 ---> 11
            }
            if (board[i][j] == 0 && lives == 3) { // previously dead but back alive
